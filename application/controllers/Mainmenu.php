...
class Mainmenu extends CI_Controller{
	...
	$record=  $this->mcrud->getList($this->tables,$config['per_page'],$page,'nama_mainmenu','ASC')->result();
        ...
        $group_id        =array('data'=>'Level','width'=>85);
	$this->table->set_heading($nomber,'Nama Mainmenu',$group_id,$link,$icon,$aktif,$option);
	foreach ($record as $r) 
            {
                $pk=  $this->pk;
    ...
    function post()
    {
        if(isset($_POST['submit']))
        {
            ...
            $data   =   array('nama_mainmenu'=>$nama,'icon'=>$icon,'link'=>$link,'aktif'=>'y','group_id'=>$this->ion_auth->get_users_groups()->row()->id);
    ...
    function edit()
    {
        if(isset($_POST['submit']))
        {
            ...
            $data   =   array('nama_mainmenu'=>$nama,'icon'=>$icon,'link'=>$link,'group_id'=>$this->ion_auth->get_users_groups()->row()->id);
